        ...
        for (Field column : columns) {
            try {
                ...
                if (fieldValue != null
                    ...
                    values.put(com.maogu.htclibrary.orm.Utils.toSQLName(column.getName()), JSON.toJSONString(fieldValue));
                }
            } catch (IllegalAccessException e) {
                throw new DataAccessException(e.getLocalizedMessage());
            }
            ...
            try {
                ...
                if (fieldValue != null
                    ...
                    values.put(Utils.toSQLName(column.getName()), String.valueOf(fieldValue));
                        ...
                        .equals("java.util.List"))) {
                    // 如果该字段继承自BaseModel；也可以保存到数据库中，转化为Json字符串保存
                    values.put(Utils.toSQLName(column.getName()), JSON.toJSONString(fieldValue));
                } else {
                    // 如果没有只，用空覆盖
